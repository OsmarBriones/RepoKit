...
- **Testing**: follows the 3-tier strategy in `REPO_MODS_METHODOLOGY.md` §8 (Tier 1: decoupled unit tests via `dotnet test`, Tier 2: Harmony reflection contract verification, Tier 3: in-game test harness / debug hotkeys). The local execution workflow deploys via `PostBuild` to Steam and the **r2modman** `Debug` profile (`AppData\Roaming\r2modmanPlus-local\REPO\profiles\Debug\BepInEx\plugins`). A `Thunderstore Mod Manager` install also exists with a `Multimancos` profile for group multiplayer tests.
